...
class TestOptions(BaseOptions):
    def __init__(self):
        ...
        parser.add_argument(
            "--n_points", required=True, type=float, help="number of points to be refined at each stage"
        )
        parser.add_argument("--smooth_kernel", default=16, type=int, help="kernel size to Gaussian blur error scores")
        parser.add_argument(
            "--save_pred",
            action="store_true",
            help="save predictions, each image will contains: image, ground-truth, coarse pred, fine pred",
        )
